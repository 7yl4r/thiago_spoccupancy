...
for(j in 1:length(AllSpp)){
  ...
  ## Export output
  sink(file = paste("0MultiOutput_Cold_Sp", j,".txt", sep = ""))
  print(AllSpp[j])
  summary(out.sp.int)
  sink(file = NULL)
  ...
  ## Plot mean occupancy
  file_name_mean = paste("0MultiMean_Cold_", j, ".jpeg", sep="")
  ...
  ## Plot SD
  file_name_sd = paste("0MultiSD_Cold_", j, ".jpeg", sep="")
  SD.Psi.Plot <- ggplot() +
    ggtitle(AllSpp[j]) +
  ...
  ## Export z posteriors
  write.table(Z_sp, file = paste("Zposterior_Cold_Sp", j,".txt", sep = ""))
