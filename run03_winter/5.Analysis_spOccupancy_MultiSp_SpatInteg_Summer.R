...
for(j in 1:length(AllSpp)){
  j=1
  ## Fill winter months (Nov to April)
  SpDetectHistory_Tel <- spDetectList_Tel[[j]][,c(11:12,1:4)]
  SpDetectHistory_Obis <- spDetectList_Obis[[j]][,c(11:12,1:4)]
  ...
  ##### Run model
  occ.formula.int <- ~  scale(Depth) + scale(SST_w) +  I(scale(SST_w)^2) +
    scale(Chlor_w) + I(scale(Chlor_w)^2) + scale(TSM_w) + scale(SSH_w)
  ...
  out.sp.int <- spIntPGOcc(occ.formula = occ.formula.int,
                           ...
                           n.chains = 3,
                           n.thin = n.thin,
                           n.omp.threads = 127, # Within chain parallel running
                           batch.length = batch.length,
                           n.report = 500)
  
  ## Export output
  sink(file = paste("Output_Winter_", AllSpp[j],".txt", sep = ""))
  print(AllSpp[j])
  summary(out.sp.int)
  sink(file = NULL)
  
  #saveRDS(out.sp.int, file = paste("ModelOutput_fullCold", AllSpp[j],".rds", sep = ""))
  
  Waic <- waicOcc(out.sp.int)
  write.table(Waic, file = paste("testeWAIC_winter_", AllSpp[j],".txt", sep = ""))
  ...
  ## Export output
  sink(file = paste("Bayesian_p-valueFullwinter_", AllSpp[j],".txt", sep = ""))
  print(AllSpp[j])
  summary(ppc.out.g1)
  sink(file = NULL)

  pdf(file = paste("FullwinterGOF2_", AllSpp[j],".pdf", sep = ""))
  ...
  # 
  # write.table(ModelValid, file = "kFoldCrossResultsWinter.txt")
  ...
  SST.pred <- (Grid_OccEnv$SST_w - mean(data.int$occ.covs[, 4])) / sd(data.int$occ.covs[, 4])
  Chlor.pred <- (Grid_OccEnv$Chlor_w - mean(data.int$occ.covs[, 7])) / sd(data.int$occ.covs[, 7])
  TSM.pred <- (Grid_OccEnv$TSM_w - mean(data.int$occ.covs[, 10])) / sd(data.int$occ.covs[, 10])
  SSH.pred <- (Grid_OccEnv$SSH_w - mean(data.int$occ.covs[, 16])) / sd(data.int$occ.covs[, 16])
  ...
  file_name_shape = paste("SDM_Shape_Winter_", spName_shape, ".shp", sep="")
  st_write(plot.grid, file_name_shape, append = FALSE)

  ## Plot mean occupancy
  file_name_mean = paste("Mean_Winter_", AllSpp[j], ".jpeg", sep="")
  ...
  ## Plot SD
  file_name_sd = paste("SD_Winter_", AllSpp[j], ".jpeg", sep="")
  SD.Psi.Plot <- ggplot() +
    ggtitle(AllSpp[j]) +
  ...
  ## Export z posteriors
  write.table(Z_sp, file = paste("Zposterior_Winter_Sp", AllSpp[j],".txt", sep = ""))
